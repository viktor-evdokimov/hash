...
impl<'p> Parameter<'p> {
    pub fn from_value(
        ...
        match ty {
            ...
            ParameterType::BaseUrl => value
                .as_str()
                .and_then(|text| BaseUrl::new(text.to_owned()).ok())
                .map(Parameter::Text)
                .ok_or_else(|| ParameterConversionError {
                    actual: ActualParameterType::Value(value.clone()),
                    expected: ty,
                }),
            ParameterType::VersionedUrl => value
                .as_str()
                .and_then(|text| VersionedUrl::from_str(text).ok())
                .map(Parameter::Text)
                .ok_or_else(|| ParameterConversionError {
                    actual: ActualParameterType::Value(value.clone()),
                    expected: ty,
...
impl From<Parameter<'_>> for ActualParameterType {
    fn from(value: Parameter<'_>) -> Self {
